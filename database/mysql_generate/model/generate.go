...
func (g *Generate) generateStruct() {
	...
	for _, f := range g.tableInfo.Fields {
		...
		s := fmt.Sprintf("%s\t%s\t `db:\"%s\" jsonpb:\"%s,omitempty\"`  // %s\n", filedName, filedType, f.Field, f.Field, strings.Trim(f.Comment, " "))
...
func (g *Generate) generateGetOne() {
	fd := `
    // exps 支持 map[string]interface{} 或 goqu 表达式（eq: exp.NewExpressionList(exp.AndType).Append(goqu.C(k).Eq(v))）
...
func (g *Generate) Write() error {
	...
	dir := _dir + "/" + strings.ToLower(g.structName[0:1]) + g.structName[1:]
	if _, _err := os.Stat(dir); _err != nil {
		if err := os.MkdirAll(dir, 0755); err != nil {
			return err
		}
	}
	file := dir + "/" + strings.ToLower(g.structName[0:1]) + g.structName[1:] + ".go"
	var f *os.File
